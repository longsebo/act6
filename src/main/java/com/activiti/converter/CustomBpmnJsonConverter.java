 ...
 */
public class CustomBpmnJsonConverter extends com.activiti.converter.BpmnJsonConverter {
...
//        }
        ...
        if ((model.getPools().size() > 0) && poolHasDI) {
            ...
            // 扩展bpmn解析器
            Map<String, ModelInfo> map1 = new HashMap<>();
            Map<String, ModelInfo> map2 = new HashMap<>();
            processFlowElements(model.getMainProcess(), model, shapesArrayNode, map1,map2,0.0, 0.0);
            processMessageFlows(model, shapesArrayNode);
        }
    ...
    @Override
    public void processFlowElements(FlowElementsContainer container, BpmnModel model, ArrayNode shapesArrayNode, Map<String, ModelInfo> map, Map<String, ModelInfo> map1, double subProcessX, double subProcessY) {
    ...
    @Override
    public void processJsonElements(JsonNode shapesArrayNode, JsonNode modelNode, BaseElement parentElement, Map<String, JsonNode> shapeMap, Map<String, String> map1, Map<String, String> map2, BpmnModel bpmnModel) {
            ...
            if (STENCIL_POOL.equals(stencilId)) {
                ...
                for (JsonNode laneNode : laneArrayNode) {
                    ...
                    if (STENCIL_LANE.equals(laneStencilId)) {
                        ...
                        processJsonElements(laneNode.get(EDITOR_CHILD_SHAPES), modelNode, lane, shapeMap,map1,map2, bpmnModel);
...
//            }
            ...
            if(StringUtils.isNotEmpty(processId)) {
                ...
                extensionElement1.setValue(processId);
                process.addAttribute(extensionElement1);
            }
            //设置扩展属性
            setProcessExtendProperties(modelNode, process);
            ...
            Map<String, String> map1 = new HashMap<>();
            Map<String, String> map2 = new HashMap<>();
            processJsonElements(shapesArrayNode, modelNode, process, shapeMap, map1,map2,bpmnModel);
        } else {
            ...
            for (JsonNode shapeNode : shapesArrayNode) {
                if (STENCIL_SEQUENCE_FLOW.equalsIgnoreCase(BpmnJsonConverterUtil.getStencilId(shapeNode))
                    ...
                    if (sourceRef != null) {
                        ...
                        CustomSequenceFlowJsonConverter flowConverter = new CustomSequenceFlowJsonConverter();
        ...
        Map<String, com.activiti.converter.BpmnJsonConverter.FlowWithContainer> allFlowMap = new HashMap<String, com.activiti.converter.BpmnJsonConverter.FlowWithContainer>();
                ...
                for (ExtensionElement orderElement : orderList) {
                    ...
                    if (StringUtils.isNotEmpty(flowValue)) {
                        if (allFlowMap.containsKey(flowValue)) {
                            com.activiti.converter.BpmnJsonConverter.FlowWithContainer flowWithContainer = allFlowMap.get(flowValue);
     ...
     * @param process
     */
    private void setProcessExtendProperties(JsonNode modelNode, Process process) {
        List<String> exincludeProperties = new ArrayList<>();
        exincludeProperties.add("process_id");
        exincludeProperties.add("signaldefinitions");
        exincludeProperties.add("messagedefinitions");
//        exincludeProperties.add("process_namespace");
        exincludeProperties.add("executionlisteners");
        exincludeProperties.add("eventlisteners");
        exincludeProperties.add("process_executable");
        exincludeProperties.add("name");
        exincludeProperties.add("messages");
        exincludeProperties.add("documentation");
        if (modelNode.get("properties") != null) {
            ...
            while(it.hasNext()){
                ...
                if(!exincludeProperties.contains(propertyName)){
                    ExtensionAttribute extensionElement1 = new ExtensionAttribute();
    ...
    private void postProcessElements(FlowElementsContainer parentContainer,
                                     ...
                                     Map<String, com.activiti.converter.BpmnJsonConverter.FlowWithContainer> allFlowMap,
        ...
        for (FlowElement flowElement : flowElementList) {
            ...
            if (flowElement instanceof BoundaryEvent) {
                ...
                if ((sourceFlowElement != null) && sourceFlowElement instanceof FlowNode) {
                    com.activiti.converter.BpmnJsonConverter.FlowWithContainer flowWithContainer = new BpmnJsonConverter.FlowWithContainer(sequenceFlow, parentContainer);
    ...
    private void readEdgeDI(Map<String, JsonNode> edgeMap,
        ...
        for (String edgeId : edgeMap.keySet()) {
            ...
            GraphicInfo sourceInfo = bpmnModel.getGraphicInfo(BpmnJsonConverterUtil
