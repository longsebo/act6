...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
    ...
    public static final String BPMN_NAMESPACE = "http://activiti.org/bpmn";
...
//        }
        //扩展流程的描述及作者
        if (StringUtils.isNoneEmpty(mainProcess.getAttributeValue(BPMN_NAMESPACE, PROPERTY_PROCESS_AUTHOR))){
            propertiesNode.put(PROPERTY_PROCESS_AUTHOR, mainProcess.getAttributeValue(BPMN_NAMESPACE, PROPERTY_PROCESS_AUTHOR));
        }
        if (StringUtils.isNoneEmpty(mainProcess.getAttributeValue(BPMN_NAMESPACE, PROPERTY_DOCUMENTATION))){
            propertiesNode.put("activiti:"+PROPERTY_DOCUMENTATION, mainProcess.getAttributeValue(BPMN_NAMESPACE, PROPERTY_DOCUMENTATION));
        }
